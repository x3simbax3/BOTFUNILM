...
INITIAL_MIGRATION = next((PROJECT_ROOT / "migrations").glob("*_initial.sql"))
...
class DatabaseTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_upsert_media_inserts_and_updates(self) -> None:
        ...
        row = await get_media_by_tmdb(42, "movie", database_url=self.database_url)

        self.assertEqual(updated_id, media_id)
        self.assertEqual(row["title"], "New title")
        self.assertEqual(row["rating"], 8.5)

    async def test_same_tmdb_id_is_allowed_for_different_types(self) -> None:
        ...
        with self.assertRaises(RuntimeError):
            async with connection_scope(self.database_url) as connection:
                await connection.execute(
                    "INSERT INTO media (media_type, title) VALUES (?, ?)",
                    ("movie", "Rolled back"),
                )
