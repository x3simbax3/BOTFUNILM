...
Новости отправляются всем пользователям, которые нажали `/start`. Слоты идут в
09:00, 11:00, 13:00, 15:00, 17:00, 19:00 и 21:00 со случайным смещением до пяти
минут; первый слот не выходит за нижнюю границу 09:00. Пропущенные после
перезапуска слоты не догоняются. Фильтры кино/сериалов, аниме, анимации, людей
кино и индустрии перемешиваются без немедленного повтора; аниме имеет тот же
приоритет, что кино и сериалы. UUID отправленных за день хранятся в Redis и
автоматически удаляются после 22:00. Картинка первой успешной отправки
переиспользуется через Telegram `file_id`, а при ошибке изображения остаётся
текст новости со ссылкой на источник.

- Во вторник–воскресенье daily-проход выбирает только `content_format='series'`
...
- `full_length` не входит ни в одну автоматическую или ручную TV-выборку.
...
12:00, и не обращается к TMDB. `news-broadcast` вручную выбирает одну свежую
новость, применяет обычную дедупликацию и рассылает её активным пользователям.
