...
Сервис `backup` ежедневно в 03:30 по `Europe/Moscow` создаёт согласованную
SQLite-копию. Перед заменой файла он восстанавливает копию в отдельную
временную БД и выполняет `integrity_check`.
Копии находятся в `backups/` в корне проекта: `bot-mon.db` … `bot-sun.db`.
Файл текущего дня недели заменяется атомарно, поэтому хранится не более семи
копий, а новая неделя перезаписывает соответствующие слоты. Для внеплановой
проверенной копии используйте `make db-backup`.
...
| `BACKUP_TIME` | время ежедневной SQLite-копии в Compose | `03:30` |
| `BACKUP_TIMEZONE` | зона расписания резервного копирования | `Europe/Moscow` |
...
`REDIS_URL=redis://redis:6379/0`. Redis не публикует порт на хост, ограничен
64 МБ памяти и сохраняет данные в именованном volume через AOF: подтверждённые
admin-задачи переживают перезапуск Redis, а FSM по-прежнему истекает по своему TTL.
Без `REDIS_URL` локальный запуск использует память процесса, поэтому незавершённые диалоги пропадут после
перезапуска.
...
Доставка новостей и уведомлений имеет семантику at-least-once: если Telegram
принял сообщение, но SQLite не сохранил отметку о доставке, при следующем
запуске оно может быть отправлено повторно.
