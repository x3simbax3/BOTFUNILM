...
снова включить их кнопкой в главном меню. Слоты идут в
...
приоритет, что кино и сериалы. Для каждого фильтра выбираются популярные статьи
TheNewsAPI за последние семь дней. Статьи без изображения и уже отправленные за
этот период пропускаются. Картинка первой успешной отправки переиспользуется
через Telegram `file_id`; если Telegram отклоняет изображение, текстовая версия
новости не отправляется.
...
12:00, и не обращается к TMDB. `news-broadcast` вручную выбирает одну популярную
новость с изображением, применяет недельную дедупликацию и рассылает её
